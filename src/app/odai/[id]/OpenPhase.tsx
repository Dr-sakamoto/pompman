...
function CloseOdaiButton({ odaiId }: { odaiId: number }) {
  ...
  return (
    <form action={action} className="space-y-2 border-t border-line pt-4">
      ...
      <p className="text-xs text-muted">
        出題者だけが操作できます。発表すると回答も採点も締め切られ、採点を終えた人には
        誰が何を書いて誰を選んだかが公開されます（まだ採点していない人は、伏せたまま
        後から採点できます）。押さなくても、上の2本のバーのどちらかが埋まれば自動で
        発表されます。採点した人が{MIN_SCORERS_TO_CLOSE}人に届くまでは、押しても待っても
        発表されません（採点0の順位表を出さないため）。
      </p>
      <ErrorText message={state.error} />
    </form>
  );
