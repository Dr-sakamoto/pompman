...
回答も採点も、結果発表までは**匿名**。だから回答を即時公開しても「名前で選ばれる」ことは起きない。
フェーズは `open`（回答・採点中）と `closed`（結果発表）の2つだけ。
...
- 回答は投稿された時点で全員に読めるが、**`author_id` は null で返る**（誰が書いたかで判断されるのを防ぐ）
...
初期版は「回答受付中は他人の回答が読めない」を RLS で担保していた。理由は
**他人の回答を見てから書くと引きずられた回答が混ざり、教師データとして劣化する**から。
0007 でこれをやめ、回答は投稿された瞬間に全員へ公開している。多様な回答から選ぶのが目的なのに
1人1回答では母数が人数で頭打ちになる、というトレードとして受け入れた判断。
**picks 側の非公開は維持している**ので、同調による汚染は引き続き防いでいる。
...
`answers_view` は security definer なので基底テーブルの RLS を迂回する。同じ可視条件をビューにも
持たせてあるので、**`answers_select` ポリシーとビューは必ず一緒に直すこと**。
...
| `/odai/new` | お題を出す |
| `/odai/[id]` | `open` なら回答フォームと採点を1画面に、`closed` なら結果を出す |
