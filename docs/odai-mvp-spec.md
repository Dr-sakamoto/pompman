...
1. 誰かがお題を出す
2. 参加者が回答を書く（**1人何個でも。出した瞬間から全員に見える**）
3. 集まった回答から各自がベスト3を選ぶ。**回答と採点は同じフェーズで同時に進む**
4. 出題者が締め切ると結果が出る
...
- **誰が書いたかは結果発表まで伏せたまま**なので、回答をその場で公開しても「名前で選ばれる」ことは起きない。
  回答受付と採点を時間で分ける必要がない。
- 分けないほうが、回答した直後に採点でき、回答数もその場で見える。手が止まらない。

引き換えに手放したものは 4.3 に書いてある。
...
理由: 全て無料枠で足りる。Supabase の Row Level Security で「結果発表まで誰が書いたか・誰を選んだかが分からない」をDB層で担保できる（アプリ層だけで隠すとAPIを直接叩けば見えてしまう）。
...
- `answers` の SELECT: 全員が読める。ただし**閲覧は `answers_view` 経由に一本化**し、
  `odai.phase = 'closed'` になるまで自分以外の `author_id` は null で返す
- `answers` の INSERT: `odai.phase = 'open'` のときのみ / 1お題・1人あたり10個まで
- `picks` の INSERT: `odai.phase = 'open'` のときのみ
- `picks` の INSERT: **自分の回答は選べない**（`answers.author_id <> auth.uid()`）
- `picks` の SELECT: `odai.phase = 'closed'` 以降のみ他人のpicksが読める

**結果発表まで匿名**なのはUX上の都合ではなく、**データ品質の要件**。
誰が書いたかが見えると、回答の中身ではなく書いた人で選ばれ、picksが「人気投票」に化ける。
他人のpicksが見えると、選好データが「他人に同調しただけのログ」に化ける。

**手放したもの（0007）**: 当初は「回答受付中は他人の回答が読めない」も担保していた。
理由は *他人の回答を見てから書くと引きずられた回答が混ざり、教師データとして劣化する* から。
回答受付と採点を1フェーズに畳んだ時点で、この保証はなくなっている。
picks 側の非公開は維持しているので、同調による汚染は引き続き防いでいる。
...
- 投稿された回答はその場で全員に出る。**回答者名は結果発表まで伏せる**
